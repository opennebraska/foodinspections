...
echo "  curl -L https://get.rvm.io | bash -s stable --auto-dotfiles >/dev/null && source ~/.rvm/scripts/rvm && rvm install --default 2.1 >/dev/null && gem install bundler >/dev/null" >> ~vagrant/.profile
